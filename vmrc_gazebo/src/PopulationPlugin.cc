...
#include "vmrc_gazebo/PopulationPlugin.hh"
...
{
  gzmsg << "Population plugin loaded" << std::endl;
  ...
  gzmsg << "Objects to populate:" << std::endl;
  while (objectElem)
  {
    // Parse the time.
...
void PopulationPlugin::OnUpdate()
{
  ...
  if (this->dataPtr->elapsedEquivalentTime >= this->dataPtr->objects.front().time)
  {
    ...
    if (modelPtr)
    {
      ...
      gzerr << "Object [" << modelName << "] not found" << std::endl;
